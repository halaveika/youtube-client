...
export class YoutubeHttpService {
  ...
  constructor(private searchService:SearchService, private http: HttpClient) {
    ...
    this.searchPattern$.subscribe(pattern =>  this.getYoutubeResponse(pattern).subscribe(items=> this.searchItemsData.next(items)) );
   }
    ...
    return this.http.get<ISearchResponse>( URL_YOUTUBE_LIST, {params})
      .pipe(
        ...
        mergeMap((videoId) => {
          ...
          return this.http.get<IVideoResponse>( URL_YOUTUBE_VIDEO, { params })
          .pipe(
  ...
  getCurrentItem(id:string):ISearchItem | null {
    let item:ISearchItem | null = null;
    this.searchItemsData$.subscribe(items=> item = items.find((item) => item.id === id) as ISearchItem);
    return item;
  }
}
